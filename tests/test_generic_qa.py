...
class GenericQATests(unittest.TestCase):
    ...
    def test_default_pairs_per_run_is_ten(self) -> None:
        self.assertEqual(parse_args(["--interval-range", "0-0"]).pairs_per_run, 10)
    ...
    def test_load_and_render_prompt_includes_type_interval_and_context(self) -> None:
        template = (
            "QA Type: {qa_type}\n"
            "Timestamp: {qa_timestamp} ({qa_timestamp_ts})\n"
        ...
        self.assertIn("QA Type: activity_window", prompt)
