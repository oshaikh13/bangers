...
def build_goal_input(index: int, item: dict[str, Any]) -> dict[str, Any]:
    ...
    combined = required_string(item, "name", label)
    goals = item.get("goals")
    if not isinstance(goals, list):
...
def build_bridge_input(
    ...
    bridge = required_string(item, "name", label)
...
def bridge_time_bounds(
    ...
    created_at = min(connected_earliest) if connected_earliest else max(all_timestamps)
